...
// ============================================
// PROSES TAMBAH JADWAL MANUAL (MULTIPLE SISWA)
// ============================================
if (isset($_POST['tambah_jadwal'])) {
    ...
    if (empty($student_ids)) {
        ...
        foreach ($student_ids as $student_id) {
            ...
            if ($cek > 0) {
                $duplicate_count++;
                $siswa = fetch_single("SELECT full_name FROM users WHERE id = '$student_id'");
                $duplicate_names[] = $siswa['full_name'];
            } else {
                $query = "INSERT INTO schedules (class_id, student_id, schedule_date, day_name, shift) 
        ...
        if ($success_count > 0 && $duplicate_count == 0) {
            $message = "Berhasil menambahkan $success_count jadwal piket!";
            $message_type = "success";
        } elseif ($success_count > 0 && $duplicate_count > 0) {
            $message = "Berhasil menambahkan $success_count jadwal. $duplicate_count siswa sudah terjadwal: " . implode(', ', $duplicate_names);
            $message_type = "warning";
        } else {
            $message = "Semua siswa sudah terjadwal di tanggal dan shift yang sama!";
            $message_type = "danger";
        }
    }
}
...
<body>
    ...
    <div id="modalTambah" class="modal">
        <div class="modal-content" style="max-width: 700px;">
            <div class="modal-header">
                <h5><i class="fas fa-plus"></i> Tambah Jadwal Manual (Multiple Siswa)</h5>
            ...
            <form method="POST" action="">
                ...
                <div class="form-group">
                    ...
                    <div style="display: flex; gap: 10px; margin-bottom: 10px;">
                        <button type="button" class="btn-primary-custom" onclick="selectAllSiswa()" style="flex: 1; padding: 8px;">
                            <i class="fas fa-check-double"></i> Pilih Semua
                        </button>
                        <button type="button" class="btn-danger-custom" onclick="deselectAllSiswa()" style="flex: 1; padding: 8px;">
                            <i class="fas fa-times"></i> Batal Semua
                        </button>
                    </div>
                    <div id="student_list_manual" style="max-height: 300px; overflow-y: auto; border: 2px solid #e0e0e0; border-radius: 8px; padding: 15px; background: #f8f9fa;">
                        <p style="text-align: center; color: #999; margin: 20px 0;">
                            <i class="fas fa-arrow-up"></i> Pilih kelas terlebih dahulu
                        </p>
                    </div>
                    <small style="color: #999; display: block; margin-top: 8px;">
                        <i class="fas fa-info-circle"></i> Centang siswa yang akan dijadwalkan piket (bisa lebih dari 1)
                    </small>
                </div>
                
                <div class="alert alert-info" style="margin-top: 15px;">
                    <i class="fas fa-lightbulb"></i> 
                    <strong>Tips:</strong> Anda bisa memilih beberapa siswa sekaligus untuk dijadwalkan di tanggal yang sama. Contoh: Senin ada 5 siswa piket, centang 5 siswa lalu simpan.
                </div>
    ...
    <script>
        ...
        // Load siswa berdasarkan kelas (untuk tambah manual dengan checkbox)
        function loadSiswaManual(classId) {
            const studentContainer = document.getElementById('student_list_manual');
            
            if (!classId) {
                studentContainer.innerHTML = '<p style="text-align: center; color: #999; margin: 20px 0;"><i class="fas fa-arrow-up"></i> Pilih kelas terlebih dahulu</p>';
                return;
            }
            
            // Fetch siswa dari server menggunakan AJAX
            studentContainer.innerHTML = '<p style="text-align: center; color: #999; margin: 20px 0;"><i class="fas fa-spinner fa-spin"></i> Loading...</p>';
            
            fetch('get-siswa.php?class_id=' + classId)
                .then(response => response.json())
                .then(data => {
                    if (data.length === 0) {
                        studentContainer.innerHTML = '<p style="text-align: center; color: #999; margin: 20px 0;"><i class="fas fa-user-slash"></i> Tidak ada siswa di kelas ini</p>';
                        return;
                    }
                    ...
                    data.forEach(siswa => {
                        html += `
                            <label style="display: flex; align-items: center; padding: 10px; background: white; border-radius: 6px; cursor: pointer; transition: all 0.3s;" 
                                ...
                                <div style="flex: 1;">
                                    <div style="font-weight: 500; color: #333;">${siswa.full_name}</div>
                                    <div style="font-size: 12px; color: #999;">NIS: ${siswa.nis}</div>
                                </div>
                            </label>
                        `;
                    });
                ...
                .catch(error => {
                    ...
                    studentContainer.innerHTML = '<p style="text-align: center; color: #dc3545; margin: 20px 0;"><i class="fas fa-exclamation-circle"></i> Error loading data</p>';
                });
        }
        
        // Select all siswa
        function selectAllSiswa() {
            const checkboxes = document.querySelectorAll('#student_list_manual input[type="checkbox"]');
